...
export class GlobalizationMock extends Globalization {
    ...
    getPreferredLanguage(): Promise<{ value: string; }> {
        let theResult: { value: string } = { value: '' };
    ...
    stringToDate(dateString: string, options: {
        ...
        let theResult: { year: number, month: number, day: number, hour: number, minute: number, second: number, millisecond: number } = { year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0, millisecond: 0 };
        return new Promise((resolve, reject) => {
            resolve(theResult);
        });
    }
    ...
    getNumberPattern(options: {
        ...
        let theResult: { pattern: string, symbol: string, fraction: number, rounding: number, positive: string, negative: string, decimal: string, grouping: string } = { pattern: '', symbol: '', fraction: 0, rounding: 0, positive: '', negative: '', decimal: '', grouping: '' };
        return new Promise((resolve, reject) => {
            resolve(theResult);
        });
    }
    ...
    getCurrencyPattern(currencyCode: string): Promise<{
        ...
        let theResult: { pattern: string, code: string, fraction: number, rounding: number, decimal: number, grouping: string } = { pattern: '', code: '', fraction: 0, rounding: 0, decimal: 0, grouping: '' };
        return new Promise((resolve, reject) => {
            resolve(theResult);
        });
    }
